...
def train_net(net,
    ...
    n_val = int(len(dataset) * val_percent) if val_percent < 1 else int(val_percent)
    n_train = len(dataset) - n_val
    if split_seed:
        train, val = random_split(dataset, [n_train, n_val], generator=torch.Generator().manual_seed(split_seed))
    else:
        train, val = random_split(dataset, [n_train, n_val])
    if type(net) == nn.parallel.DistributedDataParallel:
        train_loader = DataLoader(train, batch_size=batch_size, shuffle=False, num_workers=0, pin_memory=True, sampler=DistributedSampler(train))
        val_loader = DataLoader(val, batch_size=batch_size, shuffle=False, num_workers=0, pin_memory=True, drop_last=True, sampler=DistributedSampler(val))
    else:
...
def get_args():
    ...
    parser.add_argument('-l', '--learning_rate', metavar='LR', type=float, nargs='?', default=0.0001,
...
def select_device(device='', batch_size=None):
    ...
    s = f'UNet torch {torch.__version__} '
...
if __name__ == '__main__':
    ...
    dir_checkpoint = osp.join(".", "checkpoints", f"{current_time}_{socket.gethostname()}_"+comment)
    ...
    try:
        ...
        net = net_type(n_channels=1, n_classes=1, bilinear=False)
    except KeyError:
        os._exit(0)
    ...
    #     net = AttU_Net(n_channels=1, n_classes=1, bilinear=True).to(device=device)
    # else:
    #     os._exit(0)
