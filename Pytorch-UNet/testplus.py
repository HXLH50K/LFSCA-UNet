...
if __name__ == "__main__":
    ...
    for i, fn in enumerate(in_files):
        ...
        r = np.logical_and(true_mask, 1-pred_mask)
        g = np.logical_and(true_mask, pred_mask)
        b = np.logical_and(1 - true_mask, pred_mask)
        a = 0.3
        # # im[..., 3] = a
        # im = (im * 255).astype(np.uint8)
        # a = np.broadcast_to(img, (512,512,3))
        # img = Image.fromarray(img)
        # im = Image.fromarray(im)
        # merge = Image.blend(a, im, 0.3)
        # merge.save(osp.join(osp.dirname(args.model), "img", f"{main_id}.png"))
        mask = np.zeros((512, 512, 3))
        mask[..., 0] = r
        mask[..., 1] = g
        mask[..., 2] = b
        apl = np.logical_or(pred_mask, true_mask)[..., np.newaxis]
        plt.axis('off')
        img1 = plt.imshow(img, cmap='gray', alpha=1)
        img1 = plt.imshow(mask*apl, cmap='gray',alpha=0.3)
        os.makedirs(osp.join(osp.dirname(args.model), "img"), exist_ok=True)
        plt.savefig(osp.join(osp.dirname(args.model), "img", f"{main_id}.png"), figsize=(10, 10))
